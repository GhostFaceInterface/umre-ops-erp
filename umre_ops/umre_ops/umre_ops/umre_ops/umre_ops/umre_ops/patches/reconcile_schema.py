...
def execute():
    ...
    if _has_table(t_tour):
        ...
        if _col_exists(db, t_tour, "tur_adı") and not _col_exists(db, t_tour, "tur_adi"):
            ...
            if idx:
                _run(f"ALTER TABLE `{t_tour}` DROP INDEX `tur_adı`")
            _run(
                f"ALTER TABLE `{t_tour}` CHANGE COLUMN `tur_adı` `tur_adi` varchar(140) NULL"
            )
            _run(f"ALTER TABLE `{t_tour}` ADD UNIQUE KEY `tur_adi` (`tur_adi`)")
        ...
        for col in to_drop:
            if _col_exists(db, t_tour, col):
                _run(f"ALTER TABLE `{t_tour}` DROP COLUMN `{col}`")
    ...
    if _has_table(t_u):
        ...
        if _col_exists(db, t_u, "doğum_tarihi"):
            _run(f"ALTER TABLE `{t_u}` DROP COLUMN `doğum_tarihi`")
    ...
    if _has_table(t_b):
        if _col_exists(db, t_b, "diyanet_karti_var") and _col_exists(
            ...
            _run(f"ALTER TABLE `{t_b}` DROP COLUMN `diyanet_karti_var`")
    ...
    if _has_table(t_p):
        ...
        for col in ("vize_tipi", "maliyet_kalemleri"):
            if _col_exists(db, t_p, col):
                _run(f"ALTER TABLE `{t_p}` DROP COLUMN `{col}`")
    ...
    if _has_table(t_v):
        if _col_exists(db, t_v, "kural_kodu"):
            _run(f"ALTER TABLE `{t_v}` DROP COLUMN `kural_kodu`")
